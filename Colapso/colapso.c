...
La correccion sugerida de la fuerza de gravitacion para distancias muy pequeñas aun no ha sido implementada*/
...
/*Las posiciones se inicializan usando una distribucion esferica uniforme de masas. En total se inicializan 10000 puntos.*/
...
void initialize_pos(FLOAT *x, FLOAT *y, FLOAT *z, int n_points, FLOAT radius){
  ...
  for(i=0;i<n_points*3;i++){
    xt=(2*(rand()%lim)-lim)*radius/100000.0;
    yt=(2*(rand()%lim)-lim)*radius/100000.0;
    zt=(2*(rand()%lim)-lim)*radius/100000.0;
    u=sqrt(pow(xt,2)+pow(yt,2)+pow(zt,2));
    if(u<=radius){
      x[count]=xt;
      y[count]=yt;
      z[count]=zt;
  ...
  /*Las velocidades se inicializan como 0 dadas las especificaciones del problema.*/
...
/*Las masas de todos los elementos son iguales, asi quese inicializan con la unidad de masa.*/
...
/*La implementacion de la funcion runge_kutta4 permite actualizar la posicion de las particulas*/
